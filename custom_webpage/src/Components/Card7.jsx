...
import { cards2, Feedback } from '../data/Data';
...
function Card7() {
    ...
    for (let i = 0; i < Feedback.length; i += itemsPerSlide) {
            ...
            <div className='pt-5 md:pt-[60px] lg:pt-[120px] pb-5 md:pb-[50px] lg:pb-[150px] bg-[#ffffff]'>
                <div className=''>
                    <div className='mt-5 pb-[30px] flex flex-col justify-center items-center'>
                        ...
                        Our Fantastic Envato Customers Reviews</p>
                    </div>
                    <Carousel activeIndex={index} onSelect={handleSelect} indicators={false} controls={false} slide={false} fade>
                        {
                            groupedFeedback.map((group, i) => (
                                <Carousel.Item key={i}>
                                    <div className="flex justify-center space-x-5 md:my-11 sm:px-2 md:px-4 lg:px-0">
                                        {
                                            group.map(({ icon, iconContent, iconSecondContent, title, content, rating }, j) => (
                                                <div key={j} className='bg-[#F6F6F6] py-[41px] px-[39px] min-h-[375px] max-w-[370px]'>
                                                    <div className='flex gap-2 items-center'>
                                                        <img src={icon} alt="" />
                                                        <div>
                                                            <div>{iconContent}</div>
                                                            <div className='text-[#f9004d]'>{iconSecondContent}</div>
                                                        </div>
                                                    </div>
                                                    <h5 className='mb-[15px] mt-2 text-[#1f1f25] font-[500]'>{title}</h5>
                                                    <p>{content}</p>
                                                    <img src={rating} alt="rating" className='mt-6 max-h-[35px]' />
                                                </div>
                                            ))
                                        }
                                    </div>
                    ...
                    <div className='flex justify-center items-center space-x-5 mt-5'>
                        {
                            groupedFeedback.map((_, i) => (
                                <button key={i} className={`indicators ${i === index ? 'active' : ''}`} onClick={() => setIndex(i)}>
                                </button>
                            ))
                        }
                    </div>
